...
| `list_resources` | The model-callable twin of `resources/list` — this pod's fixed entry resources + the real-URI template. Returns metadata object with `resources` array (fixed entries like `lws-storage`, `pod-info`, `skills`) and `templates` array (the real-URI template). | none (fixed, public shape) |
