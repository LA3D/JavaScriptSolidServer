...
The MCP endpoint is `POST /mcp` on your pod, speaking JSON-RPC 2.0 over MCP's Streamable HTTP transport (protocol version `2025-03-26`). The `initialize` result advertises both `tools` and `resources` capabilities.
